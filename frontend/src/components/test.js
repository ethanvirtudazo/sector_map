...
const GicsTreeChart = () => {
  ...
  useEffect(() => {
    ...
    const dx = 20; // horizontal spacing between sibling nodes
    const dy = 180; // vertical spacing between levels
    ...
    function update(source) {
      ...
      node.append("circle")
        .attr("fill", d => d.children ? "#555" : d._children ? "#999" : "#ccc")
        .attr("r", 4)
        .on("click", (event, d) => {
          if (d.children) {
            d._children = d.children;
      ...
      node.append("text")
        .attr("dy", "0.31em")
        .attr("y", d => d.children || d._children ? -6 : 6)
